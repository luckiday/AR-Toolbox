...
//            byte[] header_sps = {0, 0, 0, 1, 103, 66, -128, 40, -38, 1, -32, 8, -97, -106, 82, 10, 4, 4, 13, -95, 66, 106}; // 1920*1080
    ...
    fun onFrame(buf: ByteArray?, offset: Int, length: Int) {
//        Log.d(TAG, "New Frame to the decoder! FrameType:" + buf[4] + " length:" + length);
        val inputBufferIndex = mCodec!!.dequeueInputBuffer(100)
        if (inputBufferIndex >= 0) {
            val inputBuffer = mCodec!!.getInputBuffer(inputBufferIndex)
            inputBuffer!!.clear()
            inputBuffer.put(buf, offset, length)
            // TODO: Check if the System.currentTimeMillis() need to be changed
    ...
    private inner class JitterThread : Thread() {
        override fun run() {
            while (true) {
//                Log.d(TAG, "JitterThread Runing:" + H264Queue.size());
                if (H264Queue.size > 25) {
                    val frameBuf = H264Queue.poll()
                    onFrame(frameBuf, 0, frameBuf.size)
                    try {
                        sleep(jitterFast)
                    } catch (e: Exception) {
...
//                    Log.d(TAG, "JitterThread Runing:" + H264Queue.size());
        ...
        init {
            Log.d(TAG, "JitterThread Created")
        }
    }
...
//                    onFrame(content.getImmutableArray(), 0, content.getImmutableArray().length);
        ...
        init {
            Log.d(TAG, "DecodeThread Created")
        }
    }
...
//                            frameName.append("P");
                        ...
                        // Append the camera info to the frame
                        var q = nodeInfo.worldRotation
                        frameName.append(java.lang.String.valueOf(q.x))
                        frameName.append(java.lang.String.valueOf(q.y))
                        frameName.append(java.lang.String.valueOf(q.z))
                        frameName.append(java.lang.String.valueOf(q.w))
...
//                            Log.v(TAG, "Receive Blob Size:" + content.size());
                        ...
                        if (interestQueue.peek() === holInterest.get() && System.currentTimeMillis() - holCounter.get() > 1000) {
                            ...
                            holCounter.set(System.currentTimeMillis())
                        }
                        if (holInterest.get() !== interestQueue.peek()) {
                            holInterest.set(interestQueue.peek())
                            holCounter.set(System.currentTimeMillis())
                        }

                        // Maybe we can process the events with another thread?
                        face!!.processEvents()

                        // We need to sleep for a few milliseconds so we don't use 100% of the CPU.
                        sleep(interestInterval)
                    }
                }
    ...
    companion object {
        ...
        private const val h264queuesize = 60
        var H264Queue = ArrayBlockingQueue<ByteArray>(h264queuesize)
