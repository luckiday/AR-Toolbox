...
public class SegmentFetcher implements OnData, OnDataValidationFailed, OnTimeout, OnNetworkNack {
    ...
    public static class Options {
         ...
         */
        public boolean useConstantInterestTimeout = true;
    ...
    (Face face, Interest baseInterest, Options options, VerifySegment verifySegment,
    ...
    (Face face, Interest baseInterest, KeyChain validatorKeyChain, Options options, Validator validator,
     VerifySegment verifySegment, OnComplete onComplete,
     OnError onError) {
...
//        fetchSegmentsInWindow(1);
    ...
    private void fetchSegmentsInWindow(int length) {
        if (checkAllSegmentsReceived()) {
            // All segments have been retrieved
//            Log.d("jNDN", "checkAllSegmentsReceived: "+ baseInterest_.getName());
            finalizeFetch();
            return;
        }

        double availableWindowSize = length;
        Map<Long, Boolean> segmentsToRequest = new HashMap(); // The boolean indicates whether a retx or not
//        Log.d("SegmentFetcher", "fetchSegmentsInWindow: availableWindowSize "+availableWindowSize);
        ...
        while (availableWindowSize > 0) {
            ...
            } else if (nSegments_ == -1 || nextSegmentNum_ < length) {
        ...
        for (Map.Entry<Long, Boolean> segment : segmentsToRequest.entrySet()) {
            ...
            Interest interest = new Interest(baseInterest_);
            interest.setName(interest.getName().appendSegment(segment.getKey()));
...
//        Log.d("jNDN", "sendInterest: "+segmentNum);
        int timeout = options_.useConstantInterestTimeout ? options_.maxTimeout : getEstimatedRto();

        if (isRetransmission) {
//            Log.d("SegmentFetcher", "sendInterest: isRetrans "+interest.getName().toString()+" "+segmentNum);
...
//            Log.d("SegmentFetcher", "sendInterest: FirstTime"+interest.getName().toString()+" "+segmentNum);
...
//        Log.d("jNDN", "onData: "+data);
        ...
        if (!currentSegmentComponent.isSegment()) {
            onError_.onError
                    (ErrorCode.DATA_HAS_NO_SEGMENT, "Data Name has no segment number");
            return;
        }
        ...
        if (receivedSegments_.size() > 0) {
            if (receivedSegments_.containsKey(segmentNum) || !pendingSegments_.containsKey(segmentNum)){
                Log.d("SegmentFetcher", "onData: Receive the same interest"+data.getName().toString());
                return;
            }
...
//                    Log.d("SegmetnFetcher", "onVerified: versionedDataName_: "+data.getName().toString()+" Length:"+nSegments_);
                } catch (EncodingException ex) {
                    try {
                        onError_.onError
                                (ErrorCode.DATA_HAS_NO_SEGMENT,
...
//            Log.d("jNDN", "onVerified: "+ data.getName());
        }
    ...
    private boolean checkMaxTimeout(){
        if (System.currentTimeMillis() >= timeLastSegmentReceived_ + options_.maxTimeout) {
            ...
            try {
                onError_.onError
                        (ErrorCode.INTEREST_TIMEOUT,
                                "Timeout exceeded: "+baseInterest_.getName().toString());
            } catch (Throwable ex) {
                logger_.log(Level.SEVERE, "Error in onError", ex);
            }
            stop();
