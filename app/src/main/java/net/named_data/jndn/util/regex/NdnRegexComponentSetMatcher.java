...
public class NdnRegexComponentSetMatcher extends NdnRegexMatcherBase {
  ...
  private int
  extractComponent(int index) throws Error
  {
    int lcount = 1;
    int rcount = 0;
    ...
    while (lcount > rcount) {
      if (index >= expr_.length())
        throw new Error("Error: angle brackets mismatch");
  ...
  private void
  compileSingleComponent() throws Error
  {
    int end = extractComponent(1);

    if (expr_.length() != end)
      throw new Error("Component expr error " + expr_);
    else {
