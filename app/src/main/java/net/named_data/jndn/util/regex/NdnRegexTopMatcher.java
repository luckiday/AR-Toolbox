...
public class NdnRegexTopMatcher extends NdnRegexMatcherBase {
  ...
  public NdnRegexTopMatcher(String expr) throws Error
  {
    super(expr, NdnRegexExprType.TOP);
    expand_ = "";
  ...
  public boolean
  match(Name name, int offset, int len) throws Error
  {
    return match(name);
  }

  public Name
  expand(String expandStr) throws Error
  {
    ...
    while (offset[0] < expand.length()) {
      ...
      if (item.charAt(0) == '\\') {
        ...
        if (0 == index) {
          ...
          throw new Error("Exceed the range of back reference");
      }
    }
  ...
  fromName(Name name, boolean hasAnchor) throws Error
  {
    String regexStr = "^";
  ...
  public static NdnRegexTopMatcher
  fromName(Name name) throws Error
  {
    ...
    throws Error
  {
    ...
    if (expand.charAt(offset[0]) == '\\') {
      ...
      if (offset[0] >= expand.length())
        throw new Error("Wrong format of expand string!");
