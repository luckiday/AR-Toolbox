...
public class KeyChain {
   ...
   * @throws LocatorMismatchError if the supplied TPM locator does not
   * match the locator stored in the PIB.
   */
  public KeyChain(String pibLocator, String tpmLocator, boolean allowReset)
    throws Error, PibImpl.Error, SecurityException, IOException
  {
    isSecurityV1_ = false;
   ...
   */
  public KeyChain()
    throws SecurityException, Error, PibImpl.Error, IOException
  {
    isSecurityV1_ = false;
  ...
  createKey(PibIdentity identity, KeyParams params)
    throws Tpm.Error, TpmBackEnd.Error, PibImpl.Error, Pib.Error, Error
  {
  ...
  sign(Data data, WireFormat wireFormat)
    throws SecurityException, TpmBackEnd.Error, PibImpl.Error, Error
  {
    if (isSecurityV1_) {
  ...
  sign(Interest interest, SigningInfo params, WireFormat wireFormat)
    throws PibImpl.Error, Error, TpmBackEnd.Error
  {
    Name[] keyName = new Name[1];
    Signature signatureInfo = prepareSignatureInfo(params, keyName);
  ...
  sign(Interest interest, WireFormat wireFormat)
    throws PibImpl.Error, Error, TpmBackEnd.Error, SecurityException
  {
    if (isSecurityV1_) {
   ...
   */
  public final CertificateV2
  selfSign(PibKey key) throws PibImpl.Error, Error, TpmBackEnd.Error
  {
   ...
   * @throws Error certificate.getKeyName() key does not exist, if the
   ...
   * @throws Error if the private key cannot be imported, or if a
  ...
  importSafeBag(SafeBag safeBag, ByteBuffer password)
    throws Error, CertificateV2.Error, TpmBackEnd.Error, PibImpl.Error,
      Pib.Error
  {
    ...
    if (tpm_.hasKey(keyName))
      throw new Error("Private key `" + keyName.toUri() +
        "` already exists");

    try {
      PibIdentity existingId = pib_.getIdentity(identity);
      existingId.getKey(keyName);
      throw new Error("Public key `" + keyName.toUri() +
        "` already exists");
    } catch (Pib.Error ex) {
    ...
    if (!VerificationHelpers.verifySignature(content, signatureBits, publicKey)) {
      tpm_.deleteKey_(keyName);
      throw new Error("Certificate `" + certificate.getName().toUri() +
...
//    Logger.getLogger(this.getClass().getName()).log(Level.INFO, "Enter Verify");
    ...
    throws Error
  {
    ...
    if (!getPibFactories().containsKey(pibScheme[0]))
      throw new Error("PIB scheme `" + pibScheme[0] +
        "` is not supported");
  }
    ...
    throws SecurityException, Error
  {
    ...
    if (!getTpmFactories().containsKey(tpmScheme[0]))
      throw new Error("TPM scheme `" + tpmScheme[0] +
        "` is not supported");
  }
   ...
   */
  private static Pib
  createPib(String pibLocator) throws Error, PibImpl.Error
  {
    String[] pibScheme = new String[1];
    String[] pibLocation = new String[1];
    parseAndCheckPibLocator(pibLocator, pibScheme, pibLocation);
   ...
   */
  private static Tpm
  createTpm(String tpmLocator) throws SecurityException, Error
  {
    String[] tpmScheme = new String[1];
    String[] tpmLocation = new String[1];
    parseAndCheckTpmLocator(tpmLocator, tpmScheme, tpmLocation);
  ...
  prepareSignatureInfo(SigningInfo params, Name[] keyName)
    throws PibImpl.Error, InvalidSigningInfoError, Error
  {
