...
public class AndroidSqlite3Pib extends PibSqlite3Base {
  ...
  public AndroidSqlite3Pib
    (String databaseDirectoryPath, String databaseFilename) throws Error
  {
   ...
   * @throws Error if initialization fails.
   */
  public AndroidSqlite3Pib(String databaseDirectoryPath) throws Error
  {
   ...
   * @throws Error for a non-semantic (database access) error.
   */
  public void
  setTpmLocator(String tpmLocator) throws Error
  {
    if (getTpmLocator().equals("")) {
      ...
      if (database_.insert("tpmInfo", null, values) < 0)
        throw new Error("AndroidSqlite3Pib: SQLite error");
    }
    else {
   ...
   */
  public boolean
  hasIdentity(Name identityName) throws Error
  {
   ...
   */
  public void
  addIdentity(Name identityName) throws Error
  {
   ...
   */
  public void
  removeIdentity(Name identityName) throws Error
  {
    ...
    try {
      while (cursor.moveToNext()) {
        ...
        try {
          ...
          throw new Error("PibSqlite3: Error decoding name: " + ex);
        }
   ...
   */
  public void
  setDefaultIdentity(Name identityName) throws Error
  {
    byte[] identityBytes = identityName.wireEncode().getImmutableArray();
   ...
   */
  public Name
  getDefaultIdentity() throws Pib.Error, Error
  {
   ...
   */
  public boolean
  hasKey(Name keyName) throws Error
  {
    // Use a statement because it allows binding a blob for the query.
   ...
   */
  public void
  addKey(Name identityName, Name keyName, ByteBuffer key) throws Error
  {
    ...
    if (!hasDefaultKeyOfIdentity(identityName)) {
      try {
        ...
        throw new Error("PibSqlite3: Error setting the default key: " + ex);
      }
    }
  }
   ...
   */
  public void
  removeKey(Name keyName) throws Error
  {
   ...
   */
  public Blob
  getKeyBits(Name keyName) throws Pib.Error, Error
  {
   ...
   */
  public HashSet<Name>
  getKeysOfIdentity(Name identityName) throws Error
  {
   ...
   */
  public Name
  getDefaultKeyOfIdentity(Name identityName) throws Pib.Error, Error
  {
    if (!hasIdentity(identityName))
      throw new Pib.Error
   ...
   */
  public boolean
  hasCertificate(Name certificateName) throws Error
  {
    // Use a statement because it allows binding a blob for the query.
   ...
   */
  public void
  addCertificate(CertificateV2 certificate) throws Error
  {
    ...
    try {
      if (cursor.moveToNext()) {
        ...
        try {
          ...
          throw new Error("PibSqlite3: Error decoding certificate: " + ex);
        }
        return certificate;
      }
      else
  ...
  private boolean
  hasDefaultIdentity() throws Error
  {
    // Use a statement because it allows binding a blob for the query.
    SQLiteStatement statement = database_.compileStatement
  ...
  private boolean
  hasDefaultKeyOfIdentity(Name identityName) throws Error
  {
    // Use a statement because it allows binding a blob for the query.
    SQLiteStatement statement = database_.compileStatement
  ...
  private boolean
  hasDefaultCertificateOfKey(Name keyName) throws Error
  {
    // Use a statement because it allows binding a blob for the query.
    SQLiteStatement statement = database_.compileStatement
